...
const foreignLangauge = require('../model/foreignLanguage');

const responseObject = new responseObjectClass();

const createForeignLanguage = catchAsync(async (req, res, next) => {
  let {
    user: { guid },
    body: {
  ...
  const addForeignLanguage = await foreignLangauge.create(foreignlangaugeObject);
  console.log('addForeignLanguage : ================================>>>>>>>>>>>>>>>>>>>>>>>',addForeignLanguage)
